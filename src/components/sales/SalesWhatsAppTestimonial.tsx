...
export default function SalesWhatsAppTestimonial() {
  ...
  useEffect(() => {
    ...
    const run = () => {
      ...
      const t3 = setTimeout(run, last + RESTART_DELAY);
          ...
          <div className="flex justify-center">
            ...
            <div className="relative w-80 md:w-[22rem]">
              <div className="rounded-[2.5rem] border-[6px] border-foreground/20 bg-[#0b141a] shadow-2xl shadow-primary/10 overflow-hidden">
                ...
                <div
                  ref={chatRef}
                  className="h-[22rem] overflow-hidden px-3 py-3 space-y-1.5"
                ...
                >
                  {conversation.slice(0, visibleMessages).map((msg, i) => (
                    ...
                    >
                      <div
                        className={cn(
                          "max-w-[82%] px-3 py-1.5 text-[13px] leading-relaxed relative",
                          msg.role === "user"
                            ? "bg-[#005c4b] text-white rounded-lg rounded-tr-sm"
                            : "bg-[#1f2c34] text-white/90 rounded-lg rounded-tl-sm"
                        )}
                      >
                        <span>{msg.text}</span>
                  ...
                  {isTyping && (
                    <div
                      className={cn(
                        "flex animate-fade-in",
                        isTyping === "user" ? "justify-end" : "justify-start"
                      )}
                    >
                      <div
                        className={cn(
